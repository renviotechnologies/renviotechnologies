        ...
        <div className="relative w-full h-full overflow-hidden">
            ...
            >
                {duplicatedLogos.map((logo, index) => (
                    <motion.div
                        key={`${logo}-${index}`}
                        className="relative w-32 h-24  bg-black/80 rounded-xl shadow-lg p-4 flex items-center justify-center flex-shrink-0 group hover:shadow-2xl transition-all duration-300"
                        whileHover={{
                            scale: 1.1,
